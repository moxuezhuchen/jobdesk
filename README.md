...
contract is `confflow>=1.5,<2.0`; CI validates against the released 1.5.0 wheel. Versions must
...
JobDesk never imports ConfFlow's contract module. ConfFlow 1.5.0 emits
...
# python -m pip install /path/to/confflow-1.5.0-py3-none-any.whl
python -m pip install -e ".[chem]"
```

```bash
# Linux compute node: install the same approved ConfFlow 1.5.0 wheel.
