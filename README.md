...
contract is `confflow>=1.4.3,<2.0`; CI validates against the 1.4.3 wheel. Versions must
...
# python -m pip install /path/to/confflow-1.4.3-py3-none-any.whl
python -m pip install -e ".[chem]"
```

```bash
# Linux compute node: install the same approved ConfFlow 1.4.3 wheel.
...
ConfFlow capability schema 3 with a compatible `>=1.4.3,<2.0` version, the
