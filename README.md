...
contract is `confflow>=1.4.5,<2.0`; CI validates against the released 1.4.5 wheel. Versions must
...
JobDesk never imports ConfFlow's contract module. ConfFlow 1.4.5 emits
...
# python -m pip install /path/to/confflow-1.4.5-py3-none-any.whl
python -m pip install -e ".[chem]"
```

```bash
# Linux compute node: install the same approved ConfFlow 1.4.5 wheel.
...
ConfFlow capability schema 4 with a compatible `>=1.4.5,<2.0` version, the
