...
> Status: design plus authoritative published acceptance evidence (2026-08-09). This document does not authorize Phase F. The current canonical evidence set is two released-v1.5.3 JobDesk control computations through the supported launcher and producer worker, plus one published-consumer v1.4.6 legacy closeout computation. The canonical control bundles are `C:\tmp\jobdesk-control-release-v153-20260809-a32\evidence.json` and `C:\tmp\jobdesk-control-release-v153-20260809-a36\evidence.json`; the legacy bundle is `C:\tmp\jobdesk-legacy-release-v146-20260809-a5\evidence.json`. The a36 bundle adds the complete persisted reconnect, fixed-cursor replay, terminal-empty-page, idempotency, cancel/resume, manifest/download, and SHA-256 lifecycle trace; the evidence index is the machine-readable aggregation boundary. A complete measured published compatibility cycle is still open, so the formal decision remains **COMPATIBILITY PERIOD CONTINUES** and Phase F is not ready. All canonical bundles retain `phase_f_ready=false`; retain both backends, the v1.4.6 rollback path, and all fail-closed gates. The evidence below remains deliberately separated into direct producer probes, real external-program probes, and JobDesk lifecycle samples.
...
The SSH/SFTP steps below remain gated by a fresh attempt root and evidence capture for any future cycle. Results are recorded separately for `control` and `legacy`; the current compatibility record has two authoritative real v1.5.3 JobDesk control-computation samples (a32 and a36) and one authoritative stable v1.4.6 rollback/legacy closeout sample. The direct candidate worker/g16 evidence remains candidate-only and does not count as a stable JobDesk computation. The a32 bundle is canonical for its persisted first-page trace; a36 is canonical for the complete persisted lifecycle, including fixed-cursor replay and the terminal empty next page. The separately retained, acceptance-failed/non-counted a34 trace remains protocol provenance only, and a35 remains non-counted after its harness failure; a5 is the current legacy closeout record.
