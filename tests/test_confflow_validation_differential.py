...
When ConfFlow is installed (the CI runner installs the pinned v2.1.6 wheel),
...
EXPECTED_ACCEPTED_BY_BOTH: list[dict[str, Any]] = [
    ...
    pytest.param(
        # ConfFlow v1.5.0 accepts whole-number floats; the offline subset
...
#     ConfFlow v1.5.0 rejects. The offline subset is more permissive.
#   * ``accepted_by_confflow_only`` — ConfFlow v1.5.0 accepts,
...
KNOWN_DIVERGENCE: list[dict[str, Any]] = [
    ...
    pytest.param(
        # ConfFlow v1.5.0 tolerates ``cores_per_task: True`` (python
    ...
    pytest.param(
        # ConfFlow v1.5.0 calls ``int(4.5) == 4`` for ``cores_per_task``
    ...
    pytest.param(
        # ConfFlow v1.5.0 checks that ``gaussian_path`` / ``orca_path``
...
def test_module_skips_are_never_silent():
    ...
    ConfFlow v2.1.6 wheel as part of the workflow's pinned install step, so a
    skip here means the
