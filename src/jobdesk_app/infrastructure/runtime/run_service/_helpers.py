...
def _tasks_from_plan(plan: RunPlan) -> list[TaskRecord]:
    ...
    return [
        TaskRecord(
            ...
            remote_stats_path=task.remote_stats_path,
            remote_log_path=task.remote_log_path,
            remote_result_paths=list(task.remote_result_paths),
            ...
            dry_run_command=task.dry_run_command,
            ...
            confflow_executable=task.confflow_executable,
        )
...
def _scheduler_type(scheduler) -> str:
    ...
    from jobdesk_app.infrastructure.remote.scheduler import PBSAdapter, SlurmAdapter
