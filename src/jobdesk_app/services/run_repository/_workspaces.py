...
def list_workspace_roots(connection: sqlite3.Connection) -> list[Path]:
    ...
    rows = connection.execute("SELECT workspace_root FROM workspace_roots ORDER BY workspace_root").fetchall()
    return [Path(str(row["workspace_root"])) for row in rows]


def delete_operation_workspace(connection: sqlite3.Connection, operation_id: str) -> Path | None:
    """Return the independently recorded workspace for a delete operation."""
    row = connection.execute(
        "SELECT workspace_root FROM delete_operation_workspaces WHERE operation_id = ?",
        (operation_id,),
    ).fetchone()
    if row is None:
        return None
