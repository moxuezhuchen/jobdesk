#!/usr/bin/env python3
"""Real-g16 ConFlow smoke (Phase 9G).

Stamps a bash harness into WSL via base64, runs it, and pulls the artifacts
back. Targets real Gaussian 16 (no mock), methane opt with b3lyp/6-31g(d).
"""
...
RESULTS_DIR = REPO_ROOT / "tmp60f7j8ix" / "phase9g_real_g16"
RESULTS_DIR.mkdir(parents=True, exist_ok=True)
DEST_WSL = "/tmp/phase9g_real_g16_inner.sh"
...
TMP="/tmp/confflow_phase9g_${BASHPID}"
...
echo
echo "[smoke] starting confflow (real g16)"
...
ls -la methane_confflow_work/g16_opt/backups/ 2>&1 || echo "(missing backups)"

echo
echo "[smoke] g16 .log key lines:"
LOGFILE=methane_confflow_work/g16_opt/backups/A000001.log
if [ -f "$LOGFILE" ]; then
    grep -E "SCF Done|Normal termination|Error termination|Optimization completed|Stationary point|SCF Done for" "$LOGFILE" 2>&1 | head -25
else
    echo "no $LOGFILE"
fi
...
def stamp_remote() -> None:
    ...
    wsl_helper = "/tmp/phase9g_deployer.py"
    deployer_content = (
...
def pull_artifacts(remote_tmp: str, target: pathlib.Path) -> None:
    pull_dir = "/tmp/confflow_phase9g_pull"
...
def main() -> int:
    ...
    print("[win] running confflow (real g16, methane opt, ~30s)...", flush=True)
    inner = run_inner()
    if inner.stdout:
