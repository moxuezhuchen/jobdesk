...
JobDesk 当前是公开预览项目。隔离源码候选版本为 `0.7.6`；已发布的
JobDesk 包为 `v0.7.5`，当前正式 producer 为 `v2.1.6`，本 consumer
候选严格绑定该版本。
...
| 隔离实现候选 | JobDesk `0.7.6` package-data 修复，基于已发布 `v0.7.5`，不修改 producer | 仅供审查和验收，不表示已发布或已切换端点 |
...
`0.7.6` 只表示隔离源码候选；`v0.7.5` 与 `v2.1.6` producer 表示已发布
