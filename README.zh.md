...
ConfFlow 工作流引擎是**可选**依赖。JobDesk 的 GUI 在不安装它时也能加载和运行；wizard、`WorkflowSpec` 与 `--resume` submitter 分支仅在执行 `pip install -e ".[chem]"` 后才可用，并且要求远端 Linux 计算节点安装匹配的 ConfFlow wheel。当前 JobDesk 合约是 `confflow>=1.4.2,<2.0`，CI 按 1.4.2 wheel 验证。Windows 与 Linux 之间必须保持版本一致，因为 GUI 导入的 Pydantic 模型（`confflow.core.models.GlobalConfigModel` / `CalcConfigModel`）正是远端 `confflow` 二进制所消费的。远端 capability 必须是 schema v2，并包含匹配的 `artifacts` 文件名契约。

```powershell
# Windows（JobDesk 端）
# 如果包索引没有提供化学版本，请先安装已批准的 wheel；
...
```bash
# Linux 计算节点也安装相同的 ConfFlow 1.4.2 wheel。
# 离线 wheel 流程见 docs/CONFFLOW_1_4_2_WHEEL_DEPLOYMENT.md。
python -m pip install /path/to/confflow-1.4.2-py3-none-any.whl
```
...
确认后（工作流模式），提交页会把 `workflow.yaml` 与输入文件放入本次提交独占的远端命名空间。首次输入上传前和提交阶段，JobDesk 都要求远端 ConfFlow 返回 schema 2、版本范围 `>=1.4.2,<2.0` 且 artifacts 字段逐项匹配的能力信息，并以 `--dry-run` 执行每个任务的真实命令；只有全部预检成功后才通过现有的 `nohup setsid` 调度器启动。

### SSH 断连韧性

`nohup` 与 ConfFlow 的恢复机制处理的是两类故障。`nohup` 让已经启动的进程在 SSH 控制连接断开后继续运行；首次启动不带 `--resume`。如果工作流进程之后停止或失败，用户显式重试时 JobDesk 会复用该 run 原有的隔离命名空间，并且只添加一次 `--resume`，由 ConfFlow 从持久化状态继续。watcher 重连后读取 `events.log`，并且只同步任务声明的 `.workflow_state.json` 与 `workflow_stats.json` 精确路径。
