...
或发布；已发布的 JobDesk 包为 `v0.7.2`，历史配套的 ConfFlow 包为
`v2.1.3`，当前正式 producer 为 `v2.1.6`，本候选严格绑定该版本。
